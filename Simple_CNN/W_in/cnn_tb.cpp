...
int main() {
    // Declare AXI streams
    hls::stream<AXI_VAL> image_in, c_weights_in, c_biases_in, d_weights_in, d_biases_in, predict_class;
    ...
    #pragma HLS PIPELINE
        				c_weights_in.write(float_to_axi(weight[ki][kj][c][f]));
        			}
        		}
        	}
        }
        ...
        #pragma HLS PIPELINE
        	c_biases_in.write(float_to_axi(biases[i]));
        }
    ...
    #pragma HLS PIPELINE
        		d_weights_in.write(float_to_axi(dense_weights[i][j]));
        	}
        }

        for (int i = 0; i < d_out_h; i++) {
    #pragma HLS PIPELINE
            	d_biases_in.write(float_to_axi(dense_biases[i]));
        }


    // Call the matrix multiplication function (this will process the data)
    cnn(image_in, c_weights_in, c_biases_in, d_weights_in, d_biases_in, predict_class);
